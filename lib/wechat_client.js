...
var WechatClient = module.exports = function(opts) {
  ...
  this.isShieldUser = function(user) {
    // TODO
  };
  ...
  this.addContacts = function(contactList) {
    _.each(contactList, this.addContact.bind(this));
  };
  ...
  this.delContacts = function(contactList) {
    _.each(contactList, this.delContact.bind(this));
  };
  ...
  this.initChatList = function(chatList) {
    this.chatList = _.filter(chatList.split(','), function(item) {
      return item.startsWith('@');
    }.bind(this));
    this.batchGetContact(this.chatList);
    return this.chatList;
  };

  this.addChatList = function(chatList) {
    var list = [];
    _.each(chatList, function(item) {
      var user = item.UserName;
      if (!user) {
        user = item.FromUserName != this.user.UserName ? item.FromUserName : item.ToUserName;
      }

      if (user.startsWith('@') && this.chatList.indexOf(user) < 0) {
        list.push(user);
        this.chatList.push(user);
      }
    }.bind(this));

    this.batchGetContact(list);
    return this.chatList;
  };

  this.deleteChatList = function(chatList) {
  ...
  this.printNewMsg = function(msg) {
    var data = {
      From: this.getUserNickName(msg.FromUserName),
      To: this.getUserNickName(msg.ToUserName),
      Message: msg.Content,
    };

    if (msg.ActualSender) {
      data.From += ' : ' + this.getUserNickName(msg.ActualSender);
    }
    console.log(columnify([data]));
  };

  this.statusNotifyProcess = function(msg) {
    switch(msg.StatusNotifyCode) {
      case consts.STATUS_NOTIFY_CODE.SYNC_CONV:
        this.initChatList(e.StatusNotifyUserName);
      break;

      case consts.STATUS_NOTIFY_CODE.ENTER_SESSION:
        this.addChatList([msg]);
      break;
    }
  };

  this.messageProcess = function(msgs) {
    _.each(msgs, function(msg) {
      ...
      msg.Content = msg.Content.replace(/^(@[a-zA-Z0-9]+|[a-zA-Z0-9_-]+):<br\/>/, function(_, sender) {
  ...
  this.updateChatData = function(data) {
    ...
    // delete contact
    this.delContacts(data.DelContactList);

    // update contact
    this.addContacts(data.ModContactList);

    // process msg
    this.messageProcess(data.AddMsgList);
  };
  ...
  this.batchGetContact = function(userList) {
    logger.debug('=====batchGetContact=====');

    userList = userList || this.contacts;
    var list = _.map(userList, function(item) {
      return {
...
WechatClient.prototype.listChat = function() {
  console.log('Chats:');
  _.each(this.chatList, function(user) {
    console.log('- ' + this.getUserNickName(user));
  }.bind(this));
};

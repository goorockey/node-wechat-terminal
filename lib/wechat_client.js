...
module.exports = class WechatClient extends EventEmitter {
  ...
  constructor(opts) {
    ...
    try {
      fs.closeSync(fs.openSync(this.opts.cookiesFile, 'wx'));
      this.cookies = request.jar(new FileCookieStore(this.opts.cookiesFile));
    } catch (e) {
      fs.closeSync(fs.openSync(this.opts.cookiesFile, 'w'));
      this.cookies = request.jar(new FileCookieStore(this.opts.cookiesFile));
    }
  ...
  _parseBaseResponse(body, response) {
    this._saveNetworkHistory(response);
  ...
  static isMuted(user) {
    return WechatClient.isRoomContact(user) ?
      user.Statues === consts.CHATROOM_NOTIFY.CLOSE :
  ...
  _addChatList(userList) {
    ...
    _.each(userList, (user) => {
      ...
      if (!name) {
        name = (user.FromUserName === this.user.UserName ? user.ToUserName : user.FromUserName);
      }
  ...
  _getUserNickName(userName) {
    ...
    var name = this.contacts[userName].RemarkName || this.contacts[userName].NickName;
  ...
  _printNewMsg(msg) {
    if (WechatClient.isMuted(msg.FromUserName)) {
      return;
    }
    ...
    _.each(msgs, (msg) => {
      ...
      switch (Number.parseInt(msg.MsgType, 10)) {
        ...
        case consts.MSG_TYPE.VOICE:
          console.log(`New message (From=${this._getUserNickName(msg.FromUserName)}, ` +
                      `Type=${_.findKey(consts.MSG_TYPE, _.matches(msg.MsgType))})`);
          break;
  ...
  searchContact(input) {
    ...
    _.each(this.contactList, (user, index) => {
      if (user.RemarkName.match(input)) {
        console.log(`#${index} ${user.RemarkName}`);
      } else if (user.NickName.match(input)) {
        console.log(`#${index} ${user.NickName}`);
      }
    });
  }
